...
window.onload = function()
{
	...
	var _container;
	var _camera,_scene;
	var _pointLight,_renderer,_material,_geometry;
	...
	function init2()
	{
		...
		{
			_value = 0.1;
			_material = new THREE.MeshBasicMaterial( { color: 0x000000, wireframe: true } );
			_fps = 30;
		} else {
			_material = new THREE.MeshBasicMaterial( { color: 0x000000, wireframe: true, side: THREE.DoubleSide } );
		}
		//_material = new THREE.MeshBasicMaterial( { color: 0x000000, wireframe: true, side: THREE.DoubleSide } );

		var _planeGeometry = new THREE.CubeGeometry( _planeSize, _planeSize, 50 );
		_plane = new THREE.Mesh( _planeGeometry, _material );
		_plane.rotation.x = - 90 * ( Math.PI / 180 );
		_scene.add( _plane );

		//_geometry = new THREE.SphereGeometry( 100, __ws, __hs );
		_geometry = new THREE.IcosahedronGeometry( 100, _value )
		...
		_scene.add( new THREE.AmbientLight( Math.random() * 0x202020 ) );
	...
	{
		_intervalKey = setInterval( _loop, 1000 / _fps >> 0 );
	}
